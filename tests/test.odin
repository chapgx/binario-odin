...
@(test)
test_bool :: proc(t: ^testing.T) {
	defer free_all(context.allocator)

	l := false
	lt: bool
	encoder := b.encoder_new(size_of(bool))
	e := b.encode(&encoder, l)
	testing.expectf(t, e == nil, "[encoding %s] expects e to be nil got %s", type_info_of(bool), e)
	buff := encoder->read()

	decoder := b.decoder_new(buff)
	e = decoder->decode(lt)
...
test_rune :: proc(t: ^testing.T) {
	...
	enc := b.encoder_new(size_of(rune))
	e := b.encode(&enc, r)
	testing.expectf(t, e == nil, "expecte e to be nil when encoding rune character")
	buff := enc->read()

	dec := b.decoder_new(buff)
	e = dec->decode(rr)
	testing.expectf(t, e == nil, "expects e to be nil got %s", e)
...
//
// 	dec1 := b.decoder_new(buff)
// 	e = dec1->decode(ufto_check)
...
test_int :: proc(t: ^testing.T, o: $T, $N: T, alloc := context.allocator) {
	...
	encoder := b.encoder_new(size_of(T))
	e := b.encode(&encoder, o)
	testing.expectf(t, e == nil, "[encoding %s] expects e to be nil got %s", type_info_of(T))


	encoded_data := encoder->read()
	decoder := b.decoder_new(encoded_data)
	e = decoder->decode(d)
